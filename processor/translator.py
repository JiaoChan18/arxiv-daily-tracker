...
每篇论文发起两次 LLM 请求：
  ...
  2. 摘要分析：单次调用，结构化输出四段内容——
       ...
       - Abstract (EN, 核心术语用 <mark> 标签高亮，LaTeX 公式原样保留)
       - 中文摘要（对应中文术语用 <mark> 标签高亮，LaTeX 公式原样保留）
...
ANALYSIS_SYSTEM_PROMPT = (
    ...
    "用 <mark>...</mark> HTML 标签包裹高亮，至少高亮 2 处，严禁一处都不高亮。\n"
    "4. ===ABSTRACT_ZH=== 段落【必须】将对应的中文术语/数值"
    "用 <mark>...</mark> HTML 标签包裹高亮，至少高亮 2 处，严禁一处都不高亮。\n"
    "5. 【铁律】绝对禁止使用 ** 加粗语法，唯一合法的高亮方式是 <mark> 标签。\n"
    "6. 【铁律】<mark> 标签内的 LaTeX 公式 $ 符号【严禁】丢失，"
    "必须完整保留（例如 <mark>$\\hat{\\rho}$</mark> 合法，$ 弄丢即为错误）。\n"
    "7. 【严禁】在四个分隔符段落之外输出任何额外文字。"
)
...
ANALYSIS_PROMPT_TEMPLATE = (
    ...
    "用 <mark>...</mark> 标签包裹高亮（至少 2 处）。"
    "LaTeX公式内字符原样保留；严禁使用 ** 加粗。\n\n"
    "===ABSTRACT_ZH===\n"
    ...
    "===ABSTRACT_EN===\n"
    "We present a <mark>variational quantum error mitigation</mark> framework that reduces logical error rates "
    "by <mark>47%</mark> on near-term noisy processors without additional physical qubit overhead. "
    "The method leverages <mark>zero-noise extrapolation</mark> combined with a learned noise model $\\hat{{\\rho}}$.\n\n"
    "===ABSTRACT_ZH===\n"
    "我们提出了一种<mark>变分量子错误缓解</mark>框架，在近期噪声量子处理器上将逻辑错误率降低了<mark>47%</mark>，"
    "且无需额外的物理量子比特开销。该方法结合了<mark>零噪声外推</mark>技术与学习型噪声模型 $\\hat{{\\rho}}$。\n\n"
    "---\n"
    "以下是待处理的论文摘要原文：\n\n{text}"
)
...
def _apply_marks(text: str, terms: list[str], *, case_insensitive: bool = False) -> str:
    """
    将 terms 中的每个术语在 text 中用 <mark>...</mark> 包裹（每术语首次出现）。

    LaTeX 公式保护：跳过位于 $...$ 内部的匹配，避免破坏公式结构。
    已被 <mark> 包裹的内容不会被重复处理。
    ...
    for term in terms:
        if not term or term in ("", " "):
            continue
        # 跳过已经存在的 <mark> 内容（防止重复包裹）
        if f"<mark>{term}</mark>" in text:
            continue
        ...
        for m in re.finditer(pattern, text, flags=flags):
            ...
            text = text[:start] + f"<mark>{matched_text}</mark>" + text[end:]
            break  # 每个术语只高亮首次出现
    return text
...
def _parse_analysis_output(raw: str, fallback_abstract: str) -> tuple[str, str, str]:
    ...
    混合高亮策略：
    - 优先使用 LLM 在 ABSTRACT_EN/ABSTRACT_ZH 中直接生成的 <mark> 标签；
    - 若 LLM 未生成（glm-4-flash 指令跟随限制），则用 Python 根据 KEYWORDS 段
      提取的术语进行兜底高亮，保证输出中始终有 <mark> 标签。
...
def translate_paper(paper: Paper, client: OpenAI, model: str, max_retries: int) -> Paper:
    ...
    对单篇论文执行深度价值提炼，结果写入：
      ...
      paper.abstract_en_highlighted — 原文摘要（核心术语 <mark> 高亮）
      paper.abstract_zh           — 中文摘要（核心术语 <mark> 高亮）
